        ...
        <Card className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-700 shadow-sm p-4">
          ...
          <div className="space-y-2" data-testid="fines-list">
            ...
            <div className="hidden lg:flex items-center px-4 py-2 text-xs font-semibold text-stone-400 dark:text-stone-500 uppercase tracking-wider">
              ...
              {canManageFines && <div className="w-24 text-right">Aktionen</div>}
            </div>
            {filteredFines.length > 0 ? (
              filteredFines.map((fine) => (
                <div
                  key={fine.id}
                  className="flex items-start lg:items-center justify-between p-4 rounded-xl border border-stone-100 dark:border-stone-700 bg-stone-50 dark:bg-stone-800 active:bg-stone-100 transition-colors"
                  data-testid={`fine-item-${fine.id}`}
                >
                  <div className="flex-1 min-w-0 lg:flex lg:items-center lg:gap-0">
                    {/* Mobil: gestapelt / Desktop: Spalten */}
                    <div className="flex-1 min-w-0 lg:pr-4">
                      ...
                      {fine.notes && (
                        <p className="text-xs text-stone-500 mt-0.5 line-clamp-1 lg:hidden">
                          Notiz: {fine.notes}
                        </p>
                      )}
                    </div>
                    ...
                    <p className="text-xs text-stone-400 mt-1 lg:hidden">
